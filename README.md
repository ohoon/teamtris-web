...
* ~소켓을 이용한 room, userlist, chat 기능 구현~ [2021-02-25]
* ~테트리스 대기 화면 UI 구성~ [2021-02-25]
* ~테트리스 연습모드(solo) 구현~ [2021-01-28]
* ~테트리스 개인전(single) 구현~ [2021-02-25]
* ~테트리스 팀전(doubles) 구현~ [2021-02-25]
...
**Added**
* Google OAuth 2.0으로 가져온 사용자 정보를 jwt token으로 발급해주는 API 구현
* Login with Google으로 로그인한 사용자는 jwt 토큰을 발급받아 서비스를 이용할 수 있도록 구현
* 접속자 명단에 각 사용자의 level 추가

**Changed**
* 이제 구글 로그인과 같은 OAuth 방식을 채택하여 운영할 것이므로 서버단에서 직접 관리하는 로그인/회원가입 코드를 삭제
* react의 환경변수를 사용하기 위해 .env를 gitignore에 추가
* nickname으로 name을 통일하고 쓸모없는 속성인 username 삭제
* 프로필 사진을 현재 연동된 계정의 프로필 사진으로 대체하도록 수정

### [2021-02-23]
 
**Added**
* Noto Sans 폰트를 추가 및 적용

**Changed**
* 전체적인 컴포넌트 UI의 스타일시트와 요소 위치들을 수정

### [2021-02-24]
 
**Added**
* UI에 쓰일 아이콘을 추가하기 위해 `react-icons` 라이브러리 채택
* 사용자 레벨을 보여주는 level 아이콘 추가

**Changed**
* 방 목록의 각 room item UI를 직관적으로 보이도록 수정
* 컴포넌트의 head 바 색상을 변경
...
**Changed**
* UI 변경이 미적용된 부분 재수정
* 로그인을 할 때마다 google에서 가져온 데이터를 db에 반영하도록 수정
* 방 설정 변경 시 게임 모드는 변경할 수 없도록 수정
* 팀을 선택하는 부분 style 수정

**Fixed**
* 게임이 2회 이상 진행되면 게임 결과창이 뜨지 않는 버그 수정
